...
public class Test12 {
    ...
    // 프론트 컨트롤러가 이 문자열을 메서드의 변수 타입에 마제 형변환한다.
    ...
    //  2) @ControllerAdvice 클래스에 @InitBinder를 둔다.
    ...
    // - @InitBinder가 붙은 메서드는 프론트 컨트롤러가 문자열을 특정 타입의 값으로 바꾸기 전에
    //   WebDataBinder를 추가로 설정하기 위해 호출 하는 메서드이다.
    ...
    protected void initBinder(WebDataBinder binder) {
        ...
        // 첫번째 파라미터 : 문자열을 어떤 타입으로 바꿀지 설정한다.
        // 두번째 파라미터 : 문자열을 java.util.Date 객체로 만들어준다.
        //      dateFormat : 실제로는 그작업을 SimpleDateFormat이 한다.
        //           false : 문자열 값이 비어있는 것을 허락할 것인지 여부
